...
[本チュートリアル作成者の過去の研究](https://link.aps.org/doi/10.1103/PhysRevLett.123.123201)ではこれを国内の X 線自由電子レーザー施設である SACLA を利用して行いました。さすがに本チュートリアル内で SACLA を利用するのも大変なので、本章では簡易化された数値計算をその計測実験の代わりに実施します。また、2024/3/21 時点では X 線広角散乱信号のみを扱うことにします。
...
* **DMP に記載された項目を研究活動に活かすためのアイデアを知ることができる**。   
    2024/3/21 時点での本サービスでは、DMP に記載された各項目の値のフォーマット上の有効性を検証に利用する程度となっております。将来的には、DMP に記載された管理対象データの存在性や、公開・非公開の宣言と実態の整合性の検証などに利用する、等に利用できると考えております。
